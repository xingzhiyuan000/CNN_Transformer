...
# 搭建神经网络-输出为10X10
class Wang_1024(nn.Module):
    def __init__(self):
        super(Wang_1024, self).__init__()
        ...
        # 64X16-64X16  通道X信号尺寸
        self.conv4 = nn.Conv1d(in_channels=64, out_channels=512, kernel_size=3, stride=1, padding=1)
...
if __name__ == '__main__':
    ...
    model=CNN_Transformer() #实例化网络模型
    model=model.to(device) #将模型转移到cuda上
    input=torch.ones((1,1,1024)) #生成一个batchsize为64的，1个通道的10X10 tensor矩阵-可以用来检查网络输出大小
    input=input.to(device) #将数据转移到cuda上
    output=model(input) #将输入喂入网络中进行处理
    print(output.shape)
    summary(model,input_size=(1,1024)) #输入一个通道为1的10X10数据，并展示出网络模型结构和参数
